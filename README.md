...
```
**4. Скачайте образы reddis и rabbitMq**
```bash
docker pull redis
docker pull rabbitmq
```
**5. Запустите контейнери reddis и rabbitMq**
```bash
docker run -it --rm --name redis -p 6379:6379 redis
docker run -it --rm --name rabbitmq -p 5672:5672 -p 15672:15672 rabbitmq:management
```
**6. Ноходясь в корне проекта (где находится manage.py), выполните команду для запуска Celery:**
```bash
...
```
**8. Запустите Stripe CLI для прослушивания веб-перехватчиков:**
```bash
stripe listen --forward-to http://localhost:8000/payment/webhook/
```
