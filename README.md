...
| **Фронтенд** | HTML, CSS, Bootstrap, JavaScript (AJAX для лайков, асинхронная добавление отзывов в окно для просмотра без перезагрузки всей страницы) |
...
**Шаг 2. Настройте параметры в файле `.env` (который находится в главной папке проекта) и base.py для запуска локально:**
...
Добавьте в файл `.env`(который находится в главной папке проекта) и  base.py для локального запуска:
...
**Получение секрета веб-перехватчика**
...
```
**4. Загрузка данных:** 
В той же папке выполните команду для загрузки тестовых данных в базу данных:
```bash
docker compose exec web python manage.py loaddata myshop_data.json
```
Логин и пароль от админки:
- Логин: alpha
- Пароль: admin2002

**5. Запустите Stripe CLI для прослушивания веб-перехватчиков:**
```bash
stripe listen --forward-to http://localhost:8000/payment/webhook/
```

**6. Сайт доступен по адресу:** `http://localhost:8000`
...
git clone https://github.com/sultonovmuhibulloh612/django-shop.git
```
**2. Заполните base.py для работы  Stripe(см. раздел "Предварительные требования") и smtp-сервер для отправки email-уведомлений.**
